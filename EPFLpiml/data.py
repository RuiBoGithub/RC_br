...
from EPFLpiml.util import normalize, inverse_normalize, ensure_list, flatten_unique_list
...
class DataSet:
    ...
    def check_columns(self, data_kwargs: dict):
        ...
        column_groups = [
            ("Case", data_kwargs["case_column"]),
            ("Target", data_kwargs["temperature_column"]),
            ("RC prediction", data_kwargs["power_column"]),
            ("Outside temperature", [data_kwargs["out_column"]]),
        ]

        if data_kwargs["neigh_column"] is not None:
            column_groups.append(
                ("Neighbouring temperature", data_kwargs["neigh_column"])
            )

        column_summary = [
            (
                label,
                [self.X_columns[i] for i in indices],
            )
            for label, indices in column_groups
        ]
        ...
        if inputs_D and isinstance(inputs_D[0], list):
            input_names = [
                [self.X_columns[i] for i in indices]
                for indices in inputs_D
            ]
        else:
            input_names = [
                self.X_columns[i]
